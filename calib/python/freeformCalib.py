...
def multiCamCalib(umeas, xworld, cameraMats, boardRotMats, boardTransVecs, planeData, cameraData):
	...
	# 	for c in range(ncams):
	# 		R += np.linalg.solve(R_vals[:, :, c], boardRotMats[:, :, n, c])
	# 		t += np.linalg.solve(R_vals[:, :, c], boardTransVecs[:, n, c] - t_vals[:, c])

	# 	R, _ = fitRotMat(R / ncams)
	# 	t /= ncams

	# 	for c in range(ncams):
	# 		boardRotMats[:, :, n, c] = np.matmul(R_vals[:, :, c], R)  / np.linalg.norm(np.matmul(R_vals[:, :, c], R), ord=2)
	# 		boardTransVecs[:, n, c] = np.matmul(R_vals[:, :, c], t) + t_vals[:, c]

	# for n in range(nplanes):
	# 	R = np.linalg.solve(R_vals[:, :, 0], boardRotMats[:, :, n, 0])
	# 	t = np.linalg.solve(R_vals[:, :, 0], boardTransVecs[:, n, 0] - t_vals[:, 0])
