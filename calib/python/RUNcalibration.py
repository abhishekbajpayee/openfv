...
class planeData(object):
    def __init__(self,dX,dY,nX,nY,num_calib_planes,fileType, z0):
        self.dX = dX
        self.dY = dY
        self.nX = nX
        self.nY = nY
...
def parseConfigFile(configPath):
    ...
    pData = planeData(dX,dY,nX,nY,nCalPlanes,fileType, z0)
...
def selfCalibrate (umeas, pData, camData, scData, tols, bounded):
    ...
    errLog = [rep_err_mean[0], rep_err_mean[Iter]]
    
    return [P,camParams,Xworld,planeParams,rep_err, avg_error, errLog]
    

def saveCalibData(exptpath, camnames, p, cparams, X, pparams, scdata, camData, planeData, avgError, errorLog, name):
    ...
    # avgError - average calibration error of all cameras
    # errorLog - first and last mean calibration for each camera
    ...
    with open(file,'w') as f:        
        ...
        f.write(str(avgError)+'\n') 
        f.write(str(camData.sX) +' '+ str(camData.sY)+' '+ '9.5334' +'\n') #to do: find this
...
if __name__ == "__main__":
    ...
    P,camParams,Xworld,planeParams,reprojectionError,avgError, errorLog = selfCalibrate(Umeas, planeData, cameraData, 
    ...
    print('\nData saved in '+str(saveCalibData(exptPath, camIDs, P, camParams, Xworld, planeParams, sceneData,
                                               cameraData, planeData, avgError, errorLog, 'results_')))
