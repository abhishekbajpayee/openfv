function [velocityStruct] = PIV_3d_matlab_o2(fieldA, fieldB, fieldC, fieldD, dt, pivOpts)
%PIV_3D_MATLAB_O2 Second order (in time) PIV using 3D intensity fields
%   Performs windowed cross correlation of four successive intensity fields,
%   using an approach which is second order accurate in time. This is more
%   computationally expensive than first order accurate computations (such as
%   PIV_3d_matlab) as two additional cross correlations are made.
%
%   The algorithm used is:
%
%           crossCorr_o2 = (AxB + 2*BxC + CxD)/4
%
%   ...where A,B,C,D represent successive intensity fields (spaced by uniform
%   time dt), and x represents cross correlation. Note that the summation of the
%   cross-correlated windows is done in the spectral domain before performing
%   IFFT to locate the displacement of the correlation peak.
%
...
%
%   Window size can be varied and a there is a variety of false vector removal
%   functions.
%
%   One current limitation of this experimental test code is that multi-pass
%   cross correlation is not available - The window shift code causes a bug
%   (windows have to be retireved from four different places not two - this code
%   currently uses the two-position retrieval from PIV_3d_matlab).
%
% Syntax:  
%       velocityStructure = PIV_3d(fieldA, fieldB,fieldC, fieldD, dt, pivOpts)
%
% Inputs:
%
...
%
% Revison History:          20 Dec 2010         Branched from PIV_3d.m (now
%                                               PIV_3d_matlab.m)
%                           17 April 2011       Reformulated for compatibility
...
for pass = 1:pivOpts.npasses
    ...
    % Display progress
    disp(['PIV3d_2ndOrder.m: Pass ' num2str(pass) ' of ' num2str(pivOpts.npasses)])
    ...
    % win_ctrs, n_windows, start_inds and end_inds.
    
    
    % Form a meshgrid of x, y, z window centre points. This is the volume grid
    % on which the volumetric velocity data Ux, Uy and Uz will be calculated.
    ...
    if pass > 1
        ...
        % NB should consider low-pass filtering this to sort out erroneous
        % vectors. The pass frequency would be an additional parameter
        % to be tuned.
    ...
    for yctr = 1:n_windows(2)
        ...
        for xctr = 1:n_windows(1)
            ...
            for zctr = 1:n_windows(3)
                ...
                window3 = fieldC(y_inds_1, x_inds_1, z_inds_1);
                window4 = fieldD(y_inds_2, x_inds_2, z_inds_2);
                
                
                   
                % CROSS CORRELATE WINDOWS USING 3D FFTS.
                ...
                % Use second order accurate correlation plane
                corr = corr1 + 2*corr2 + corr3;
                ...
                switch lower(pivOpts.peakFinder)
                    ...
                    otherwise
                        error('TomoPIVToolbox:PIV_3d','Incorrect specifier for peak location method. Try ''cubic'' or ''gaussian''')
                end
                ...
                if plotting  && (yctr == 1) && (xctr == 1) %&& (zctr == 1)
                    
                    % Set up figure with 3 subplots:
                    raiseFigure(['PIV_3d_2ndOrder Debug Plot: yctr = ' num2str(yctr) ' xctr = ' num2str(xctr) ' zctr = ' num2str(zctr)]);
                    clf
                    subplot(1,3,1);
                    % Plot the particle distributions:
                    fld_h_1 =  patch(isosurface(window1,0.01));
                    fld_h_2 =  patch(isosurface(window2,0.01));
                    fld_h_3 =  patch(isosurface(window3,0.01));
                    fld_h_4 =  patch(isosurface(window4,0.01));
                    set(fld_h_1,'FaceColor','b','EdgeColor','none','FaceAlpha',0.7);
                    set(fld_h_2,'FaceColor','m','EdgeColor','none','FaceAlpha',0.7);
                    set(fld_h_3,'FaceColor','c','EdgeColor','none','FaceAlpha',0.7);
                    set(fld_h_4,'FaceColor','g','EdgeColor','none','FaceAlpha',0.7);
                    ...
                    bug(bug <= eps('single')) = [];
                    hist(ah2, bug,100);
                    title('Histogram of nonzero values in correlation volume')
                    ...
                    if sum(isnan(loc_inds(:,1))) == 0
                        ...
                        isoval = 0.9.*max(isomat(:));
                        pk1_iso_h = patch(isosurface(isomat,isoval));
                        set(pk1_iso_h,'FaceColor','k','EdgeColor','none','FaceAlpha',0.9);
                        
                        isomat = double(plot_real_corr(tlo(1):thi(1),tlo(2):thi(2),tlo(3):thi(3)));
                        isoval = 0.7.*max(isomat(:));
                        pk1_iso_h = patch(isosurface(isomat,isoval));
                        set(pk1_iso_h,'FaceColor','b','EdgeColor','none','FaceAlpha',0.7);
                        
                        isomat = double(plot_real_corr(tlo(1):thi(1),tlo(2):thi(2),tlo(3):thi(3)));
                        isoval = 0.3.*max(isomat(:));
                        pk1_iso_h = patch(isosurface(isomat,isoval));
                        set(pk1_iso_h,'FaceColor','c','EdgeColor','none','FaceAlpha',0.3);
                    ...
                    title('Correlation peaks and correlation plots')
                    legend({'Peak Position';'0.9 x Peak Value';'0.7 x Peak Value';'0.3 x Peak Value'})
                    xlim([0 wsize(1)/2])
                    ylim([0 wsize(2)/2])
                    zlim([0 wsize(3)/2])
                    axis equal
    ...
    % Translation of windows. Not necessary on the first pass.
%     warning('Authors note - possible error where velocity guess field is applied at this point in the code')
    ...
    if sum(sum(sum(isnan(ux) | isnan(uy) | isnan(uz)))) > 0
        error('TomoPIVToolbox:PIV_3d','Gap filling not working, as all velocities NaN. Check inputs and try again.')
    end
...
end % END FUNCTION PIV_3d_matlab_o2
