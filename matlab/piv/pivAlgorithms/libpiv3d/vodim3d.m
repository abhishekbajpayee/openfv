function [ur uc up snr] = vodim3d(fieldA, fieldB, c0, c1, c2, c3, c4, c5, c6, c7, wSize, iwMaxDisp, fetchType, parFlag)
%VODIM3D Matlab wrapper for the mex_vodim3d function
...
if ~isequal(fieldSize, size(fieldB))
    error('MATLAB:TomoPIVToolbox:IncorrectSize','Field arrays A and B are different sizes')
end
...
% Check that corner arrays do not contain NaN from incorrect interpolation
% or faulty data in the guess field
if any(isnan(c0(:))) || any(isnan(c1(:))) || any(isnan(c2(:))) || any(isnan(c3(:))) || any(isnan(c4(:))) || any(isnan(c5(:))) || any(isnan(c6(:))) || any(isnan(c7(:)))
    error('MATLAB:TomoPIVToolbox:InvalidData','Window corner location arrays contain NaN values')
end
...
% Make the direct call to the mex function
if parFlag
    [ur uc up snr] = mex_vodim3d(fieldA, fieldB, c0, c1, c2, c3, c4, c5, c6, c7, wSize, iwMaxDisp, fetchType);
else
    [ur uc up snr] = fMexPIV(fieldA, fieldB, c0, c1, c2, c3, c4, c5, c6, c7, wSize, iwMaxDisp, fetchType);
end
