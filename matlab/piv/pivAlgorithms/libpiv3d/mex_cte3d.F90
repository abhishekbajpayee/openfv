...
!
! Matlab Syntax: 
!		[ur uc up snr] = mex_vodim3d(fieldA, fieldB, fieldC, fieldD, c0, c1, c2, c3, c4, c5, c6, c7, wSize); 
!
!
! Inputs:
!
...
!     FORTRAN pointers to input/output arrays
      ...
      REAL*4, POINTER :: fieldCptr(:,:,:)
      REAL*4, POINTER :: fieldDptr(:,:,:)
...
!     Get pointers to the scalar input arguments out of the prhs array   
      xptrWSIZE     = mxGetPr(prhs(13))
      xptrIWMAXDISP = mxGetPr(prhs(14))
	  xptrFETCHTYPE = mxGetPr(prhs(15))
	  
!     Get single precision pointers to the input arrays
      fieldAptr => fpGetPr3Single(prhs(1))
      fieldBptr => fpGetPr3Single(prhs(2))
      fieldCptr => fpGetPr3Single(prhs(3))
      fieldDptr => fpGetPr3Single(prhs(4))
      c0ptr => fpGetPr2Single(prhs(5))
      c1ptr => fpGetPr2Single(prhs(6))
      c2ptr => fpGetPr2Single(prhs(7))
      c3ptr => fpGetPr2Single(prhs(8))
      c4ptr => fpGetPr2Single(prhs(9))
      c5ptr => fpGetPr2Single(prhs(10))
      c6ptr => fpGetPr2Single(prhs(11))
      c7ptr => fpGetPr2Single(prhs(12))
...
!     CALL COMPUTATIONAL SUBROUTINE  
      ...
      CALL cte3d(WSIZE, NWINDOWS, NCORRS, &
                         fieldDims(1), fieldDims(2), fieldDims(3), &
                         0, IWMAXDISP, &
                         fieldAptr, fieldBptr, fieldCptr, fieldDptr, &
                         FETCHTYPE, &
