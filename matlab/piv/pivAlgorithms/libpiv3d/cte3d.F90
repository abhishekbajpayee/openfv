...
!                               X= A,B,C,D for different snapshots 
!                               separated by time dt
!
...
!                               expected.
!                               CX is NCORRS*4 in length: The first 
!                               NCORRS rows describe windows from 
!                               intensity array A, the nex NCORRS refer 
!                               to intensity array B, the next C, the 
!                               last NCORRS rows points to windows in D.
!
...
! Inclusion of the vodim3d code (crude solution to a problem - compiling separately and linking together results in duplicate symbols in the .so. To tired to figure out the proper solution)
#include "vodim3d.F90"


      SUBROUTINE cte3d(WSIZE, NWINDOWS, NCORRS, fd1, fd2, fd3, &
                         CC_SMOOTH_FLAG, IWMAXDISP, &
                         FIELDA, FIELDB, FIELDC, FIELDD, FETCHTYPE, &
...
!     Inputs
	  ...
	  REAL*4, DIMENSION(fd1, fd2, fd3), INTENT(in) :: FIELDA, &
...
!     Internal Variables                                                     
      ...
      REAL*4, DIMENSION(WSIZE,WSIZE,WSIZE) :: WA,WB,WC,WD   ! Local arrays for the windows
...
!     violations, the first thing to do is to try raising this...
      CALL kmp_set_stacksize_s(536870912)
...
!
!     SETUP FFTS
!     This is ugly to have here but for some reason I can't make it work in a subroutine
!_______________________________________________________________________
...
!     Get the maximum number of threads
      IF (FETCHTYPE.GT.2) THEN  ! Alter this if adding parallelism to other loops - currently we parallelise some parts and not others
...
!     Set the maximum number of threads
      stat          = DftiSetValue (DescHandle, &
                               DFTI_NUMBER_OF_USER_THREADS, maxThreads)
...
!     cross correlated
      IF (FETCHTYPE.EQ.1) THEN
        ...
        ! O_M_P PARALLEL DO DEFAULT(NONE) SHARED(NCORRS, NWINDOWS, WSIZE, CC_SMOOTH_FLAG, IWMAXDISP, WEIGHT, fd1, fd2, fd3, DescHandle, DescHandleInv, C0, C1, C2, C3, C4, C5, C6, C7, FIELDA, FIELDB, FIELDC, FIELDD, UR, UC, UP, SNR, ERRCODE) PRIVATE(WA,WB,WC,WD,THREADERRCODE) SCHEDULE(DYNAMIC,10)
        DO WCTR = 1,NCORRS

            ! Retrieve Windows A and B using direct indexing
            CALL indexedFetch( WCTR,          WSIZE, NWINDOWS, &
                                fd1, fd2, fd3, &
                                C0, C1, C2, C3, C4, C5, C6, C7, &
                                FIELDA, WA)
            CALL indexedFetch( WCTR+NCORRS,   WSIZE, NWINDOWS, &
                                fd1, fd2, fd3, &
                                C0, C1, C2, C3, C4, C5, C6, C7, &
                                FIELDB, WB)
            CALL indexedFetch( WCTR+2*NCORRS, WSIZE, NWINDOWS, &
                                fd1, fd2, fd3, &
                                C0, C1, C2, C3, C4, C5, C6, C7, &
                                FIELDC, WC)
            CALL indexedFetch( WCTR+3*NCORRS, WSIZE, NWINDOWS, &
                                fd1, fd2, fd3, &
                                C0, C1, C2, C3, C4, C5, C6, C7, &
                                FIELDD, WD)

            ! Cross Correlate Window A with Window B
            CALL crossCorrCTE(WCTR, WSIZE, NCORRS, CC_SMOOTH_FLAG, &
                                IWMAXDISP, WEIGHT, DescHandle, &
                                DescHandleInv, WA, WB, WC, WD, &
                                UR, UC, UP, SNR)
            
        ENDDO
        ! O_M_P END PARALLEL DO


      ELSEIF (FETCHTYPE.EQ.2) THEN

      
        ! O_M_P PARALLEL DO DEFAULT(NONE) SHARED(NCORRS, NWINDOWS, WSIZE, CC_SMOOTH_FLAG, IWMAXDISP, WEIGHT, fd1, fd2, fd3, DescHandle, DescHandleInv, C0, C1, C2, C3, C4, C5, C6, C7, FIELDA, FIELDB, FIELDC, FIELDD, UR, UC, UP, SNR, ERRCODE) PRIVATE(WA,WB,WC,WD, THREADERRCODE) SCHEDULE(DYNAMIC,50)
        DO WCTR = 1,NCORRS
               
            ! Retrieve Windows A and B using linear interpolation
            CALL triLInterp( WCTR,          WSIZE, NWINDOWS, &
                                fd1, fd2, fd3, &
                                C0, C1, C2, C3, C4, C5, C6, C7, &
                                FIELDA, WA)
            CALL triLInterp( WCTR+NCORRS,   WSIZE, NWINDOWS, &
                                fd1, fd2, fd3, &
                                C0, C1, C2, C3, C4, C5, C6, C7, &
                                FIELDB, WB)
            CALL triLInterp( WCTR+2*NCORRS, WSIZE, NWINDOWS, &
                                fd1, fd2, fd3, &
                                C0, C1, C2, C3, C4, C5, C6, C7, &
                                FIELDC, WC)
            CALL triLInterp( WCTR+3*NCORRS, WSIZE, NWINDOWS, &
                                fd1, fd2, fd3, &
                                C0, C1, C2, C3, C4, C5, C6, C7, &
                                FIELDD, WD)
            ...
            CALL crossCorrCTE(WCTR, WSIZE, NCORRS, CC_SMOOTH_FLAG, &
                                IWMAXDISP, WEIGHT, &
                                DescHandle, DescHandleInv, &
                                WA, WB, WC, WD, UR, UC, UP, SNR)
      ...
      ELSEIF (FETCHTYPE.EQ.3) THEN
        ...
        !$OMP PARALLEL DO DEFAULT(NONE) SHARED(NCORRS, NWINDOWS, WSIZE, CC_SMOOTH_FLAG, IWMAXDISP, WEIGHT, fd1, fd2, fd3, DescHandle, DescHandleInv, C0, C1, C2, C3, C4, C5, C6, C7, FIELDA, FIELDB, FIELDC, FIELDD, UR, UC, UP, SNR, ERRCODE) PRIVATE(WA,WB,WC,WD, THREADERRCODE) SCHEDULE(DYNAMIC,50)
        DO WCTR = 1,NCORRS
            ...
            CALL whitCardInterp5( WCTR,          WSIZE, NWINDOWS, &
                                    fd1, fd2, fd3, &
                                    C0, C1, C2, C3, C4, C5, C6, C7, &
                                    FIELDA, WA)
            CALL whitCardInterp5( WCTR+NCORRS,   WSIZE, NWINDOWS, &
                                    fd1, fd2, fd3, &
                                    C0, C1, C2, C3, C4, C5, C6, C7, &
                                    FIELDB, WB)
            CALL whitCardInterp5( WCTR+2*NCORRS, WSIZE, NWINDOWS, &
                                    fd1, fd2, fd3, &
                                    C0, C1, C2, C3, C4, C5, C6, C7, &
                                    FIELDC, WC)
            CALL whitCardInterp5( WCTR+3*NCORRS, WSIZE, NWINDOWS, &
                                    fd1, fd2, fd3, &
                                    C0, C1, C2, C3, C4, C5, C6, C7, &
                                    FIELDD, WD)
      ...
      ELSEIF (FETCHTYPE.EQ.4) THEN
        ...
        !$OMP PARALLEL DO DEFAULT(NONE) SHARED(NCORRS, NWINDOWS, WSIZE, CC_SMOOTH_FLAG, IWMAXDISP, WEIGHT, fd1, fd2, fd3, DescHandle, DescHandleInv, C0, C1, C2, C3, C4, C5, C6, C7, FIELDA, FIELDB, FIELDC, FIELDD, UR, UC, UP, SNR, ERRCODE) PRIVATE(WA,WB,WC,WD, THREADERRCODE) SCHEDULE(DYNAMIC,100)
        DO WCTR = 1,NCORRS
            ...
            ! Retrieve Windows A and B using the 7^3 Whittaker Cardinal Function
            CALL whitCardInterp7( WCTR,          WSIZE, NWINDOWS, &
                                    fd1, fd2, fd3, &
                                    C0, C1, C2, C3, C4, C5, C6, C7, &
                                    FIELDA, WA)
            CALL whitCardInterp7( WCTR+NCORRS,   WSIZE, NWINDOWS, &
                                    fd1, fd2, fd3, &
                                    C0, C1, C2, C3, C4, C5, C6, C7, &
                                    FIELDB, WB)
            CALL whitCardInterp7( WCTR+2*NCORRS, WSIZE, NWINDOWS, &
                                    fd1, fd2, fd3, &
                                    C0, C1, C2, C3, C4, C5, C6, C7, &
                                    FIELDC, WC)
            CALL whitCardInterp7( WCTR+3*NCORRS, WSIZE, NWINDOWS, &
                                    fd1, fd2, fd3, &
                                    C0, C1, C2, C3, C4, C5, C6, C7, &
                                    FIELDD, WD)
