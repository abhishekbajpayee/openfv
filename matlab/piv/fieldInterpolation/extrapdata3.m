function [w] = extrapdata3(x, y, z, v, xi, yi, zi, varargin)
...
% Reshape the input data to row vectors (so that bsxfun below works)
x = reshape(x,[1 n_input]);
y = reshape(y,[1 n_input]);
z = reshape(z,[1 n_input]);
v = reshape(v,[1 n_input]);

% Use binary singleton expansion to create [n_ext x n_input] matrices:
x_dist_sqd = bsxfun(@minus,ext_xi,x).^2;
y_dist_sqd = bsxfun(@minus,ext_yi,y).^2;
z_dist_sqd = bsxfun(@minus,ext_zi,z).^2;

% The distance^2 is used as the weighting function...
weight = x_dist_sqd + y_dist_sqd + z_dist_sqd;

% Close together elements should have a higher weighting. Thus we need to take
% the 1/x of the weights
weight = 1./weight;

% We'll need to normalise back at some point. Sum elements in column direction:
weight_sum = sum(weight,2);

% Perform the weighted averaging
ext_wi = sum(weight.*repmat(v,[n_ext 1]),2)./weight_sum;
